...
extension MBProgressHudSwift{
    ...
    override func updateConstraints() {
        ...
        subviews.enumerated().forEach { (offset, element) in
            ...
            bezelConstraints?.append(contentsOf: NSLayoutConstraint.constraints(withVisualFormat: "|-(>=margin)-[view]-(>=margin)-|", options: .init(rawValue: 0), metrics: metrics, views: ["element":element]))
...
extension MBProgressHudSwift{
    ...
    class func showHud(addedToView view:UIView,withAnimated animated:Bool) -> MBProgressHudSwift{
        ...
        return MBProgressHudSwift(view: view)
    }
